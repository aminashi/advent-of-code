...
//    ieodomkazucvgmuy is naughty because it has a repeating letter with one between (odo), but no pair that appears twice.
    ...
    for word in words {
        ...
        for (i, char) in word.enumerated() {
            let index1 = word.index(word.startIndex, offsetBy: i+1)
            let char2 = word[index1]
            let array = word.components(separatedBy: "\(char)\(char2)")
            if array.count > 2 {
                firstRule = true
                break
            }
        ...
        for i in 0...word.count-3 {
            ...
            let index3 = word.index(word.startIndex, offsetBy: i+2)
            secondRule = word[index1] == word[index3]
            if secondRule {
                break
            }
        }
